...
export default function PublicMenuPage() {
  ...
  const handleRate = (itemId: string, rating: number) => {
    ...
    toast.success(`Thank you! You rated this dish ${rating} stars.`, {
      icon: "⭐",
      style: {
        borderRadius: '1rem',
        background: '#333',
                  ...
                  <div className="relative w-full h-64 rounded-[3rem] overflow-hidden shadow-2xl transition-all duration-500 border border-gray-50">
                    ...
                    <div className="absolute bottom-6 left-6 right-6">
                       ...
                       <div className="flex items-center gap-0.5">
                          {[1, 2, 3, 4, 5].map((s) => (
                            <Star key={s} className={cn("h-2 w-2", (ratedItems[item.id] || 5) >= s ? "fill-yellow-400 text-yellow-400" : "text-gray-400")} />
                          ))}
                       </div>
                    </div>
                  </div>
                  ...
                  <div className="p-10 -mt-20 relative z-10 bg-white rounded-t-[4rem]">
                     <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-1.5 bg-gray-50 px-4 py-1.5 rounded-full border border-gray-100">
                           ...
                           <span className="text-[10px] font-black text-gray-900">Highly Rated</span>
                        </div>
                     </div>

                     <h2 className="text-4xl font-black text-gray-900 tracking-tighter mb-4">{selectedItem.name}</h2>
                     <p className="text-gray-500 text-sm font-medium leading-relaxed mb-8">{selectedItem.description}</p>

                     {/* Customer Rating Section */}
                     <div className="p-8 bg-gray-50 rounded-[3rem] border border-gray-100 mb-8 text-center">
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Rate this Experience</p>
                        <div className="flex items-center justify-center gap-3">
                           {[1, 2, 3, 4, 5].map((s) => (
                             <motion.button
                               ...
                               onClick={() => handleRate(selectedItem.id, s)}
                               className="relative p-1"
                             >
                               <Star 
                                 className={cn(
                                   "h-8 w-8 transition-all duration-300",
                                   (hoveredStar !== null ? s <= hoveredStar : s <= (ratedItems[selectedItem.id] || 0))
                        ...
                        {ratedItems[selectedItem.id] && (
                           <motion.p initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-4">
                              Thanks for your {ratedItems[selectedItem.id]} star rating!
                           </motion.p>
                        )}
                     </div>
